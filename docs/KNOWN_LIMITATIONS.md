...
- `convex/_generated` is committed (Convex-recommended), so the frontend imports the typed `api` and the repo typechecks without running codegen first. The bootstrap call uses the typed `api.users.bootstrap`; a `bun run check` guard (`scripts/no-untyped-convex-refs.sh`) forbids untyped `makeFunctionReference`/`anyApi` references in `src/`. Regenerate with `bunx convex dev` (which also supports an anonymous **local** deployment needing no cloud account); never edit generated files by hand.
